with
    ...
    customers as (
        select customer_id, first_name, last_name, email, address, phone_number
    ...
    joined as (
        ...
        from customers
        left join
            customer_metrics on (customers.customer_id = customer_metrics.customer_id)
        left join
            customer_survey_responses
            on (customers.email = customer_survey_responses.customer_email)
