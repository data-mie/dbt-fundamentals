with orders as (
    ...
    from {{ ref('orders') }} --raw.ecomm.orders
), 
...
customers as (
    select
        ...
        address,
        phone_number
    from {{ ref('stg_ecomm__customers') }} --raw.ecomm.customers
),
...
joined as (
    select
        ...
        survey_responses.satisfaction_score,survey_responses.survey_date
    ...
    left join survey_responses on (customers.email = survey_responses.customer_email)
